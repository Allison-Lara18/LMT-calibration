...
2. **Two pruning methods**
   ...
   * **Local gain rule:** `regression_pruning_gain_local(...)` keeps a split only if $\frac{n_L}{n} \mathrm{AUC}_L + \frac{n_R}{n} \mathrm{AUC}_R - \mathrm{AUC}_{\text{parent}} \geq \delta$
